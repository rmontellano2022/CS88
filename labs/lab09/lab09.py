...
def computeAncesTree(t):
    ...
    "*** YOUR CODE HERE ***"
    child_eth = {}
    for par_ethnicity in t.get_ethnicities():
        for key, value in par_ethnicity.items():
            if key in child_eth:
                child_eth[key] += value/2
            else:
                child_eth[key] = value/2
                #values should be stored but also divided by 2
    for child in t.children.keys():
        child.ethnicity = child_eth

    for child in t.children.values():
        computeAncesTree(child)
